...
"""Import GraphForge, run allow-state product laws, run law tests."""
...
from calfimforge._bootstrap import ensure_paths  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        for law in discover_product_laws(REPO / "calfimforge"):
            ...
            require_law(
                ...
                law_id=str(law.get("id") or "calfim_law"),
        ...
        return 1
    tr = subprocess.run(
        [sys.executable, "-m", "pytest", str(REPO / "tests" / "test_calfimforge_laws.py"), "-q"],
        cwd=str(REPO),
        check=False,
    )
