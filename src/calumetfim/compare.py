...
"""Containment and IoU on the Munster to South Holland window."""
...
from calumetfim.window import require_not_huc_wide
...
def overlap_table(
    *,
    ...
    drain = require_not_huc_wide(drain_to_reach)
    w = np.asarray(wet)
    u = np.asarray(usgs)
    z = np.asarray(zone)
    ...
    n = int(on.sum())
    if n == 0:
        raise GateError("no comparable cells on the window")
    ...
    return {
        ...
        "iou_universe": "munster_south_holland_window",
...
def leftover_sentence(table: dict) -> str:
    ...
    miss = usgs_n - both
    p_both = int(table["n_p_and_usgs"])
    p_n = int(table["n_p_ge_t"])
    return (
        f"{both} of {usgs_n} USGS wet cells are HAND-wet ({miss} miss). "
        f"P>={float(table['p_headline_t'])}: {p_n} cells, {p_both} also USGS-wet. "
        ...
        f"IoU HAND vs USGS = {float(table['iou_hand_usgs']):.2f} on the "
        "Munster to South Holland window."
    )
