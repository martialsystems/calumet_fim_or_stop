...
Empty HAND overlap on that window is a stop, not a substitute study. Science `e11fce4`. Do not start `chicago_little_calumet_fim` from this git.

## GraphForge

Pin is `calfimforge/`. Five laws: Dunn/Straub/Manaster 2020 identity, published GRIDIDs, no Grand Calumet/INFIP, no TRI five-row, empty-overlap stop. Engine checkout `~/graphforge`. No catalog/`surfaces.json` unless the operator asks.

## Verify

`python3 ~/agent_laws_verify_before_done/vbd_gate.py check --app-root . --claim-done`

`vbd.runtime.json` runs `.venv/bin/python -m pytest`, `scripts/run_fixture.py`, and `calfimforge/scripts/sanity_calfimforge.py`. Do not use stock `/usr/bin/python3 -m pytest`.
